...
async def respond_to_message(websocket, message):
    try:
        ...
        data = { 
            'error': f'error decoding {message}',
    ...
    # DONE: Log in to web socket server
    type = data.get('type')
    if type == 'login':
        ...
        # Send message to all of the users that a client has joined
        for socket in logged_in_users:
            await socket.send(json.dumps(message))
        
        
    # DONE: When any of the clients sends a message of type "chat", sends a message to anyone logged in
    elif type == 'chat':
    ...
    # DONE: We've done the disconnect
        ...
        # Send message to all of the users that a client has left
        for socket in logged_in_users:
            await socket.send(json.dumps(message))
        print("Stop tracking this web socket / user", data)
...
async def broadcast_messages(websocket):
    ...
    # The except and finally code handles disconnects
